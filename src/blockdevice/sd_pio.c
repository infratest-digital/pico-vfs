...
#include "blockdevice/pio_spi.h"
#include "blockdevice/sd_pio.h"
#include "spi.pio.h"

typedef struct {
    pio_spi_inst_t *spi_inst;
...
static uint8_t _spi_write(void *_config, uint8_t data) {
    ...
    pio_spi_write8_read8_blocking(config->spi_inst, buffer, buffer, 1);
...
static void _spi_wait(void *_config, size_t count) {
    ...
    for (size_t i = 0; i < count; i++) {
        pio_spi_write8_read8_blocking(config->spi_inst, &SPI_FILL_CHAR, NULL, sizeof(SPI_FILL_CHAR));
    }
}
...
static void _spi_init(void *_config) {
    ...
    uint offset = pio_add_program(config->spi_inst->pio, &spi_cpha0_program);
    printf("Loaded program at %d\n", offset);

    pio_spi_init(config->spi_inst->pio, config->spi_inst->sm, offset,
...
static bool _wait_ready(void *_config, uint64_t timeout) {
    ...
    uint32_t t = to_ms_since_boot(get_absolute_time());
    do {
        pio_spi_write8_read8_blocking(config->spi_inst, &SPI_FILL_CHAR, &response, sizeof(SPI_FILL_CHAR));
        if (response == 0xFF) {
            return true;
        }
...
static uint8_t _cmd_spi(void *_config, int cmd, uint32_t arg) {
    ...
    for (int i = 0; i < PACKET_SIZE; i++) {
        pio_spi_write8_read8_blocking(config->spi_inst, (const uint8_t *)&cmd_packet[i], NULL, 1);
    }
    ...
    if (CMD12_STOP_TRANSMISSION == cmd) {
        pio_spi_write8_read8_blocking(config->spi_inst, &SPI_FILL_CHAR, NULL, 1);
    }

    // Loop for response: Response is sent back within command response time (NCR), 0 to 8 bytes for SDC
    for (int i = 0; i < 0x10; i++) {
        pio_spi_write8_read8_blocking(config->spi_inst, &SPI_FILL_CHAR, &response, 1);
...
static int _read(void *_config, uint8_t *buffer, uint32_t length) {
    ...
    // read data
    pio_spi_read8_blocking(config->spi_inst, 0xFF, buffer, length);
...
static uint8_t _write(void *_config, const uint8_t *buffer, uint8_t token, uint32_t length) {
    ...
    // write the data
    pio_spi_write8_blocking(config->spi_inst, buffer, length);
...
blockdevice_t *blockdevice_sd_pio_create(pio_spi_inst_t *spi_inst,
                                         uint8_t mosi,
                                         uint8_t miso,
                                         uint8_t sclk,
                                         uint8_t cs,
...
void blockdevice_sd_pio_free(blockdevice_t *device) {
    free(device->config);
    device->config = NULL;
    free(device);
    device = NULL;
